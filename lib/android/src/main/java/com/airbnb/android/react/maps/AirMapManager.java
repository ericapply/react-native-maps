...
public class AirMapManager extends ViewGroupManager<AirMapView> {
  ...
  private static final int ANIMATE_TO_LATLNG = 9;
  private static final int ANIMATE_TO_LATLNG_ZOOM = 10;
  ...
  public void receiveCommand(AirMapView view, int commandId, @Nullable ReadableArray args) {
    ...
    switch (commandId) {
      ...
      case ANIMATE_TO_LATLNG_ZOOM: {
        ...
        lng = position.getDouble("longitude");
        lat = position.getDouble("latitude");
        view.animateToPositionZoom(new LatLng(lat, lng), (float) zoom, duration);
        break;
      }
    }
  }
